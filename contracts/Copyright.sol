...
contract MusicCopyrightMarketplace {
    ...
    function getMyMusicCopyrights() external view returns (MusicCopyright[] memory){
        MusicCopyright[] memory myMusics = new MusicCopyright[](keys.length);
        for (uint i = 0; i < keys.length; i++) {
            if (musicCopyrights[keys[i]].current_owner == msg.sender) {
                myMusics[i] = musicCopyrights[keys[i]];
            }
        }
        return myMusics;
    }
